...
(function( $ ) {
	$( document ).on( 'sowsetupformfield', '.siteorigin-widget-field-type-taxonomy', function ( e ) {
		...
		var updateSelectedItems = function() {
			...
			$$.find( 'ul.items > li' ).each( function ( index, element ) {
				...
				if ( selectedItems.indexOf( $li.data( 'id' ) ) > -1 ) {
		...
		var refreshList = function(){
			...
			var taxonomy = $contentSearchInput.data('taxonomy');
			var ajaxData = { action: 'custom_term_search' };
			if ( source === 'posts' ) {
		...
		$$.find('.siteorigin-widget-taxonomy-input').click(function () {
