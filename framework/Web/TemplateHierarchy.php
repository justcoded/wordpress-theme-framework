...
class TemplateHierarchy
{
	...
	public function __construct()
	{
		...
		add_action('init', array($this, 'initThemePageTemplateHooks'), 1000);

		// add woocommerce support
		add_filter( 'woocommerce_template_path', array($this, 'woocommerceTemplatePath') );

		// Support of ACF, to display page templates from subfolders
		add_filter( 'acf/location/rule_values/page_template', array($this, 'acfPageTemplatesScan'), 20, 1 );
	}

	/**
	 * Add hooks for all post types for check post_templates in views folders as well
	 */
	public function initThemePageTemplateHooks()
	{
		$post_types = get_post_types();
		foreach ($post_types as $post_type) {
			add_filter("theme_{$post_type}_templates", array($this, "themePageTemplates"), 20, 4);
		}
	}
	...
	public function __call($name, $arguments)
	{
		...
		$type = $matches[1];
		return $this->extendTemplateHierarchy($type, $arguments[0]);
	}
	 ...
	 * @return array
	 */
	protected function extendTemplateHierarchy($type, $templates)
	{
		global $_jtf_templates_hierarchy_lookup;

		// generate template for post types
		if ( $type == 'single'       && $templates = $this->getSingleTemplates()  ) :
		elseif ( $type == 'page'     && $templates = $this->getPageTemplates($templates)    ) :
		elseif ( $type == 'archive'  && $templates = $this->getArchiveTemplates()  ) :
		// defaults - wrap with views folders based on type
		elseif ( $templates = $this->wrapTemplateInFolders($type, $templates) ) :
		endif;
	 ...
	 * @return array
	 */
	protected function getPageTemplates($std_templates)
	{
		$templates = array();
		if ( $custom_view = get_page_template_slug() ) {
			...
			$templates = $this->wrapTemplateInFolders('page', $std_templates);
		}
		return $templates;
	}
	 ...
	 * @return array
	 */
	protected function getSingleTemplates()
	{
	 ...
	 * @return array
	 */
	protected function getArchiveTemplates()
	{
	 ...
	 * @return array
	 */
	protected function wrapTemplateInFolders($type, $templates)
	{
		$foldered_templates = array();

		foreach ($this->template_types_folders as $folder => $folder_types) {
			if ( !in_array($type, $folder_types) ) continue;
	 ...
	 * @return array
	 */
	public function themePageTemplates($post_templates, $wp_theme, $post, $post_type)
	{
		$post_templates_depth2 = $this->scanPostTemplates(2);
	 ...
	 * @return array
	 */
	public function scanPostTemplates( $depth )
	{
		...
		if ( ! is_array( $post_templates ) ) {
			...
			foreach ($files as $file => $full_path) {
				if ( $full_path === __FILE__ ) continue;
	 ...
	 *
	 * @param string $wocommerce_path
	 * @return string
	 */
	public function woocommerceTemplatePath($wocommerce_path)
	{
	 ...
	 * @return array mixed
	 */
	public function acfPageTemplatesScan( $page_templates )
	{
